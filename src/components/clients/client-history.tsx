...
type AppointmentStatus = "waiting" | "confirmed" | "completed" | "missed" | "cancelled"
...
export function ClientHistory({ history }: { history: Appointment[] }) {
  ...
  const filtered = filter === "all" ? history : history.filter((a) => a.status === filter)
  const shown = filtered.slice(0, visible)
  const hasMore = visible < filtered.length

  function handleFilter(value: AppointmentStatus | "all") {
    setFilter(value)
      ...
      {history.length === 0 ? (
        ...
        <>
          ...
          <div className="flex flex-wrap gap-1.5">
            {FILTER_OPTIONS.filter(({ value }) =>
              value === "all" || history.some((a) => a.status === value)
            ).map(({ value, label }) => (
              <button
                key={value}
          ...
          {filtered.length === 0 ? (
            <p className="py-4 text-center text-sm text-muted-foreground">Nenhum atendimento com esse status.</p>
          ) : (
