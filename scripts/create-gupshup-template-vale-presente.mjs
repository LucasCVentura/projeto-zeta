#!/usr/bin/env node
/**
 * Cria o template kira_vale_presente na Gupshup e o submete para aprovação da Meta.
 * Header de imagem (o QR code do vale) + corpo avisando que é um presente grátis.
 *
 ...
 *
 * Uso:
 *   GUPSHUP_APP_ID=<app-id> node scripts/create-gupshup-template-vale-presente.mjs
 *
...
const content = "Ola {{1}}! Voce ganhou {{3}} da {{2}}: {{4}}, totalmente gratis! Valido ate {{5}}. Mostre o QR code acima na hora do atendimento."

const example = content
  .replace("{{1}}", "[Camila]")
  .replace("{{2}}", "[Studio da Ana]")
  .replace("{{3}}", "[um presente]")
  .replace("{{4}}", "[Limpeza de pele]")
  .replace("{{5}}", "[30/08]")

const params = new URLSearchParams({
  elementName: "kira_vale_presente",
  languageCode: "pt_BR",
  category: "UTILITY",
  templateType: "IMAGE",
  vertical: "Vale-presente pra cliente",
  content,
  example,
...
if (res.ok) {
  try {
    ...
    if (id) {
      ...
      console.log("- Admin → Config WhatsApp → campo \"Vale-presente\"")
      console.log(`- ou env GUPSHUP_TEMPLATE_KIRA_VALE_PRESENTE_ID=${id}`)
    }
